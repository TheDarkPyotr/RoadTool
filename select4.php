...
<?php if(!isset($_GET['approved'])){ 
		...
		$page = isset($_GET['approvedp']) ? $input->EscapeString($_GET['approvedp']) : 1;
		//$flag = isset($_GET['name']) ? "WHERE username LIKE '%".$input->EscapeString($_GET['name'])."%'" : "";
		$sql = mysql_query("SELECT * FROM city_report WHERE status = 'APPROVATA' AND hide LIKE 0 ORDER BY id DESC") or die();
		...
		$sql = mysql_query("SELECT * FROM city_report WHERE status = 'APPROVATA' AND hide LIKE 0 ORDER BY id DESC LIMIT $limit OFFSET $offset") or die();
		
		if($page > 1) { ?><li><a href="?approvedp=<?php echo ($page-1); ?>">&laquo;</a></li><?php } else { ?><li class="disabled"><a href="">&laquo;</a></li><?php }
			...
			if ($i < $page + 8){
				if($i == $page){ echo "<li class=\"active\"><a href=\"#\">".$i."</a></li>\n";
					...
					if ($i + 4 >= $page && $page + 4 >= $i){
						echo "<li><a href=\"?approvedp=".$i."  \">".$i."</a></li>\n";
					}
				}
			}
		}
		?>
		<?php if($page < $pages) { ?><li><a href="?approvedp=<?php echo ($page+1); ?>">&raquo;</a></li><?php }else{ ?><li class="disabled"><a href="">&raquo;</a></li><?php } ?>
		</ul>
		</header>
       ...
       <div class="tab_container">
			...
			*/
			...
			while($report = mysql_fetch_assoc($report1)){
				*/
				while($row = mysql_fetch_assoc($sql)){
					$report = mysql_fetch_assoc(mysql_query("SELECT * FROM city_report WHERE id = '".$row['id']."' AND status = 'APPROVATA' AND hide LIKE 0"));
					//SELECT * FROM city_report WHERE approved LIKE 1
				...
				echo '
				<tr> 
   					<td><a href="?approved='.$report['id'].'">'.$report['id'].'</a></td> 
    				<td><a href="?approved='.$report['id'].'">'.$input->cutString($report['report_title'],25).'</a></td> 
...
<?php  			} else echo "Nessuna richiesta in questa categoria.";   
	...
	$id = isset($_GET['approved']) ? $input->EscapeString($_GET['approved']) : '';
	$sql = mysql_query("SELECT * FROM city_report WHERE id = ".$id." AND status = 'APPROVATA' AND hide LIKE 0") or die();
