...
<?php if(!isset($_GET['priority'])){ 
		...
		$page = isset($_GET['priorityp']) ? $input->EscapeString($_GET['priorityp']) : 1;
		//$flag = isset($_GET['name']) ? "WHERE username LIKE '%".$input->EscapeString($_GET['name'])."%'" : "";
		$sql = mysql_query("SELECT * FROM city_report WHERE priority LIKE 1 AND hide LIKE 0 ORDER BY id DESC") or die();
		...
		$sql = mysql_query("SELECT * FROM city_report WHERE priority LIKE 1 AND hide LIKE 0 ORDER BY id DESC LIMIT $limit OFFSET $offset") or die();
		
		if($page > 1) { ?><li><a href="?priorityp=<?php echo ($page-1); ?>">&laquo;</a></li><?php } else { ?><li class="disabled"><a href="">&laquo;</a></li><?php }
			...
			if ($i < $page + 8){
				if($i == $page){ echo "<li class=\"active\"><a href=\"#\">".$i."</a></li>\n";
					...
					if ($i + 4 >= $page && $page + 4 >= $i){
						echo "<li><a href=\"?priorityp=".$i."  \">".$i."</a></li>\n";
					}
				}
			}
		}
		?>
		<?php if($page < $pages) { ?><li><a href="?priorityp=<?php echo ($page+1); ?>">&raquo;</a></li><?php }else{ ?><li class="disabled"><a href="">&raquo;</a></li><?php } ?>
		</ul>
		</header>
       ...
       <div class="tab_container">
			...
			*/
			...
			while($report = mysql_fetch_assoc($report1)){
				*/
				while($row = mysql_fetch_assoc($sql)){
					$report = mysql_fetch_assoc(mysql_query("SELECT * FROM city_report WHERE id = '".$row['id']."' AND priority LIKE 1 AND hide LIKE 0"));
					//SELECT * FROM city_report WHERE priority LIKE 1
				...
				echo '
				<tr> 
   					<td><a href="?priority='.$report['id'].'">'.$report['id'].'</a></td> 
    				<td><a href="?priority='.$report['id'].'">'.$input->cutString($report['report_title'],25).'</a></td> 
	...
	$id = isset($_GET['priority']) ? $input->EscapeString($_GET['priority']) : '';
	$sql = mysql_query("SELECT * FROM city_report WHERE id = ".$id." AND priority LIKE 1 AND hide LIKE 0") or die();
	
	if(mysql_num_rows($sql) > 0 && isset($id) == true){
	
	
	$row = mysql_fetch_assoc($sql);
	if($row['priority'] == 0) echo "Richiesta non prioritaria";
		else {
