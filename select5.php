...
<?php if(!isset($_GET['rejected'])){ 
		...
		$page = isset($_GET['rejectedp']) ? $input->EscapeString($_GET['rejectedp']) : 1;
		//$flag = isset($_GET['name']) ? "WHERE username LIKE '%".$input->EscapeString($_GET['name'])."%'" : "";
		$sql = mysql_query("SELECT * FROM city_report WHERE status = 'RIGETTATA' AND hide LIKE 0 ORDER BY id DESC") or die();
		...
		$sql = mysql_query("SELECT * FROM city_report WHERE status = 'RIGETTATA' AND hide LIKE 0 ORDER BY id DESC LIMIT $limit OFFSET $offset") or die();
		if(mysql_num_rows($sql) > 0){
		if($page > 1) { ?><li><a href="?rejectedp=<?php echo ($page-1); ?>">&laquo;</a></li><?php } else { ?><li class="disabled"><a href="">&laquo;</a></li><?php }
			...
			if ($i < $page + 8){
				if($i == $page){ echo "<li class=\"active\"><a href=\"#\">".$i."</a></li>\n";
					...
					if ($i + 4 >= $page && $page + 4 >= $i){
						echo "<li><a href=\"?rejectedp=".$i."  \">".$i."</a></li>\n";
					}
				}
			}
		}
		?>
		<?php if($page < $pages) { ?><li><a href="?rejectedp=<?php echo ($page+1); ?>">&raquo;</a></li><?php }else{ ?><li class="disabled"><a href="">&raquo;</a></li><?php } ?>
		</ul>
		</header>
       ...
       <div class="tab_container">
			...
			*/
			...
			while($report = mysql_fetch_assoc($report1)){
				*/
				while($row = mysql_fetch_assoc($sql)){
					$report = mysql_fetch_assoc(mysql_query("SELECT * FROM city_report WHERE id = '".$row['id']."' AND status = 'RIGETTATA' AND hide LIKE 0"));
				...
				echo '
				<tr> 
   					<td><a href="?rejected='.$report['id'].'">'.$report['id'].'</a></td> 
    				<td><a href="?rejected='.$report['id'].'">'.$input->cutString($report['report_title'],25).'</a></td> 
	...
	$id = isset($_GET['rejected']) ? $input->EscapeString($_GET['rejected']) : '';
	$sql = mysql_query("SELECT * FROM city_report WHERE id = ".$id." AND status = 'RIGETTATA' AND hide LIKE 0") or die();
