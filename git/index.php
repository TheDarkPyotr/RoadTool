...
<body>
	<div id="content">
		<?php
        $db = new DB('localhost', 'root', '', 'alcamoreport' );
		$posts = $db->getResults( "SELECT * FROM city_report ORDER BY ID DESC LIMIT 3" ); //WHERE post_type = 'post' AND post_status = 'publish'
		$allPosts = $db->getResults( "SELECT * FROM city_report  ORDER BY ID" ); // WHERE post_type = 'post' AND post_status = 'publish'
		...
		foreach( $posts as $post => $content ) {
			...
			$postTitle = $content['name'];
			$date = strtotime( $content['surname'] );
